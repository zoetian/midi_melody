...
function normalize_notes(notes)
{
	...
	{
		res.push({bdelta:notes[i].begin_delta,edelta:notes[i].end_delta,octave:Math.floor(notes[i].globalIndex/12),index:kidx_map[notes[i].globalIndex%12].index,keytype:kidx_map[notes[i].globalIndex%12].keytype});
	}
	return res;
}
